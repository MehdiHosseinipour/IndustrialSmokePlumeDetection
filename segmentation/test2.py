...
def train_model(model, epochs, opt, loss, batch_size, patience=10, val_loss_threshold=0.5):
    """Wrapper function for model training with early stopping.
    ...
    :param patience: (int) number of epochs with no improvement after which training will stop early
    :param val_loss_threshold: (float) validation loss threshold to stop training if exceeded"""
    ...
    for epoch in range(epochs):
        ...
        # Early stopping condition: stop training if validation loss exceeds threshold
        if val_loss_total/(j+1) > val_loss_threshold:
            print(f"Validation loss exceeded the threshold of {val_loss_threshold}, stopping early.")
            break
